...
import { g as getDHYProducts, c as installPlugin } from "./dhy-CpRy-bJi.js";
import "node:path";
import "pinia";
import "crypto-js";
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const fetchProducts = async () => {
      try {
        loading.value = true;
        await getDHYProducts("plugin").then((res) => {
          var _a;
          console.log(res.list);
          list.value = ((_a = res == null ? void 0 : res.list) == null ? void 0 : _a.map((v, i) => {
            var _a2;
            if ((_a2 = localPlugins.value) == null ? void 0 : _a2.length) {
              const localPluginIdx = localPlugins.value.findIndex((local) => local.name === v.key);
              if (localPluginIdx > -1) {
                const localPlugin = localPlugins.value[localPluginIdx];
                v.hasNew = v.version > localPlugin.version;
                v.installed = true;
              }
            }
            v.controls = [
        ...
        switch (key) {
          case "install":
            if (!record.installed) {
              ...
              await installPlugin({ productId: record.product.id }).then((res) => {
                control2.label = "已安装";
                message.success("安装并启动成功");
                if (res.pluginConf)
                  ssrStore.plugins.unshift(res.pluginConf);
              }).catch((e) => {
                console.error(e);
                control2.label = "安装";
                control2.class = "";
          ...
          case "update":
            ...
            await installPlugin({ productId: record.product.id }).then((res) => {
              ...
              if (res.pluginConf) {
                const idx = localPlugins.value.findIndex((v) => v.name === record.product.key);
                ssrStore.plugins[idx] = res.pluginConf;
              }
              list.value[record.productIndex].controls[1] = null;
            }).catch((e) => {
              console.error(e);
              control.label = "更新";
              control.class = "";
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/views/admin/plugin/market.vue");
