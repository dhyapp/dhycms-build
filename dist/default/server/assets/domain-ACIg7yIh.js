...
import { c as blockDomain } from "./index-0psH9gUa.js";
  ...
  setup(__props) {
    ...
    const fetchList = async () => {
      try {
        ...
        list.value = [];
        await blockDomain.list({ query }).then((res) => {
          list.value = res.list || [];
    ...
    const delHandle = async (ids) => {
      Modal.confirm({
        title: "确认移除？",
        content: "移除对象将从域名黑名单中剔除",
        centered: true,
        onOk: async () => {
          return new Promise((resolve, reject) => {
            blockDomain.del({ ids }).then(() => {
                    ...
                    default: withCtx((_2, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(unref(Button), {
                          size: "small",
                          type: "primary",
                          onClick: ($event) => onSetDomain(record)
                        }, {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/views/admin/security/domain.vue");
