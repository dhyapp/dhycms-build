...
import { Breadcrumb, BreadcrumbItem, Flex, Space, DropdownButton, Menu, MenuItem, Button, Select, SelectOption, Input, Table, Tag, Dropdown, Modal, Form, FormItem, Textarea, message } from "ant-design-vue";
import { m as menu, p as post } from "./index-0psH9gUa.js";
  ...
  setup(__props) {
    ...
    const fetchList = async () => {
      try {
        ...
        list.value = [];
        await post.list({ query }).then((res) => {
          list.value = res.list || [];
    ...
    const blockHandle = async (ids, status = "off") => {
      ...
      try {
        await post.update({ ids, body: { status } }).then(() => {
    ...
    const tagHandle = async () => {
      try {
        ...
        await post.update({ ids: selectedRowKeys.value, body: toRaw(tagForm) }).then(() => {
    ...
    const menuHandle = async () => {
      try {
        ...
        await post.update({ ids: selectedRowKeys.value, body: toRaw(menuForm) }).then(() => {
    ...
    const showStylesModal = ref(false);
    const stylesForm = reactive({ styles: null });
    const stylesModalFormRef = ref(null);
    const stylesHandle = async () => {
      try {
        batchLoading.value = true;
        await stylesModalFormRef.value.validate();
        await post.update({ ids: selectedRowKeys.value, body: toRaw(stylesForm) }).then(() => {
          message.success("样式设置成功");
          showStylesModal.value = false;
          stylesForm.styles = null;
          batchInit();
        }).catch((e) => {
          console.error(e);
          message.error("样式设置失败");
        });
      } finally {
        batchLoading.value = false;
      }
    ...
    const remarkForm = reactive({ remark: null });
    const remarkModalFormRef = ref(null);
    const remarkHandle = async () => {
      try {
        batchLoading.value = true;
        await remarkModalFormRef.value.validate();
        await post.update({ ids: selectedRowKeys.value, body: toRaw(remarkForm) }).then(() => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
                        ...
                        _push4(ssrRenderComponent(unref(Menu), { onClick: onBatchOptions }, {
                          default: withCtx((_4, _push5, _parent5, _scopeId4) => {
                            if (_push5) {
                              ...
                              _push5(ssrRenderComponent(unref(MenuItem), { key: "remark" }, {
                                default: withCtx((_5, _push6, _parent6, _scopeId5) => {
                                  if (_push6) {
                                    _push6(`设置备注`);
                                  } else {
                                    return [
                                      createTextVNode("设置备注")
                                    ];
                                  }
                                }),
                                _: 1
                              ...
                              return [
                                ...
                                createVNode(unref(MenuItem), { key: "remark" }, {
                                  default: withCtx(() => [
                                    createTextVNode("设置备注")
                                  ]),
                                  _: 1
      ...
      _push(ssrRenderComponent(unref(Table), {
        columns: [
          { title: "封面", dataIndex: "poster", align: "center", width: 80 },
          { title: "标题", dataIndex: "title", ellipsis: true },
          { title: "作者", dataIndex: "author", ellipsis: true },
          { title: "所属分类", dataIndex: "mid" },
          { title: "标签", dataIndex: "tag" },
          { title: "累计浏览", dataIndex: "viewCount" },
        ...
        bodyCell: withCtx(({ column, record }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (column.dataIndex === "poster") {
              ...
              if (record.poster) {
                _push2(`<div class="poster"${_scopeId}><img${ssrRenderAttrs(ssrGetDirectiveProps(_ctx, _directive_depic, `${record.poster}?0x30`))}${_scopeId}></div>`);
              } else {
                _push2(`<span${_scopeId}>-</span>`);
              }
              _push2(`</div>`);
            } else if (column.dataIndex === "title") {
              _push2(`<a${ssrRenderAttr("href", `/archive/${record.id}`)} target="_blank"${_scopeId}>${ssrInterpolate(record.title)}</a>`);
            ...
            } else if (column.dataIndex === "author") {
              _push2(`<!--[-->${ssrInterpolate(record.author || "-")}<!--]-->`);
            } else if (column.dataIndex === "viewCount") {
              _push2(`<!--[-->${ssrInterpolate(unref(numFormat)(record.viewCount))}<!--]-->`);
              ...
              _push2(ssrRenderComponent(unref(Tag), {
                bordered: false,
                color: record.status === "off" ? "error" : "success"
              }, {
                default: withCtx((_, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`${ssrInterpolate(mapStatus[record.status] || "上线")}`);
                  } else {
                    return [
                      createTextVNode(toDisplayString(mapStatus[record.status] || "上线"), 1)
                    ];
                  }
                }),
                _: 2
              ...
              _push2(ssrRenderComponent(unref(Space), null, {
                default: withCtx((_, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(ssrRenderComponent(_component_RouterLink, {
                      to: `/${preadmin.value}/post/edit/${record._id}`
                    }, {
            ...
            return [
              column.dataIndex === "poster" ? (openBlock(), createBlock("div", {
                key: 0,
                class: "f ac jc"
              }, [
                record.poster ? (openBlock(), createBlock("div", {
                  key: 0,
                  class: "poster"
                }, [
                  withDirectives(createVNode("img", null, null, 512), [
                    [_directive_depic, `${record.poster}?0x30`]
                  ])
                ...
                target: "_blank"
              }, toDisplayString(record.title), 9, ["href"])) : column.dataIndex === "mid" ? (openBlock(), createBlock(Fragment, { key: 2 }, [
                createTextVNode(toDisplayString(mapMenuName.value[record.mid] || "-"), 1)
              ], 64)) : column.dataIndex === "tag" ? (openBlock(), createBlock(Fragment, { key: 3 }, [
                createTextVNode(toDisplayString(record.tag || "-"), 1)
              ], 64)) : column.dataIndex === "author" ? (openBlock(), createBlock(Fragment, { key: 4 }, [
                createTextVNode(toDisplayString(record.author || "-"), 1)
              ], 64)) : column.dataIndex === "viewCount" ? (openBlock(), createBlock(Fragment, { key: 5 }, [
                createTextVNode(toDisplayString(unref(numFormat)(record.viewCount)), 1)
              ], 64)) : column.dataIndex === "_id" ? (openBlock(), createBlock(Fragment, { key: 6 }, [
                ...
                _: 2
              }, 1032, ["color"])) : column.dataIndex === "ctrl" ? (openBlock(), createBlock(unref(Space), { key: 8 }, {
                default: withCtx(() => [
                  createVNode(_component_RouterLink, {
                    to: `/${preadmin.value}/post/edit/${record._id}`
                  }, {
      ...
      _push(ssrRenderComponent(unref(Modal), {
        open: showStylesModal.value,
        "onUpdate:open": ($event) => showStylesModal.value = $event,
        title: "批量设置标题样式",
        centered: "",
        confirmLoading: batchLoading.value,
        onOk: stylesHandle
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(unref(Form), {
              ref_key: "stylesModalFormRef",
              ref: stylesModalFormRef,
              model: stylesForm,
              class: "mt20"
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(ssrRenderComponent(unref(FormItem), { name: "styles" }, {
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(unref(Textarea), {
                          value: stylesForm.styles,
                          "onUpdate:value": ($event) => stylesForm.styles = $event,
                          placeholder: "请输入CSS样式"
                        }, null, _parent4, _scopeId3));
                      } else {
                        return [
      ...
      _push(ssrRenderComponent(unref(Modal), {
        open: showRemarkModal.value,
        "onUpdate:open": ($event) => showRemarkModal.value = $event,
        title: "批量文章备注",
        centered: "",
        confirmLoading: batchLoading.value,
        onOk: remarkHandle
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(unref(Form), {
              ref_key: "remarkModalFormRef",
              ref: remarkModalFormRef,
              model: remarkForm,
              class: "mt20"
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    ...
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(unref(Textarea), {
                          ...
                          "onUpdate:value": ($event) => remarkForm.remark = $event,
                          placeholder: "请备注"
                        }, null, _parent4, _scopeId3));
                      } else {
                        return [
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/views/admin/post/index.vue");
